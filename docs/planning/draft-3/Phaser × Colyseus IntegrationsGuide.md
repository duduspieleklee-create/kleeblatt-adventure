...
# ## **6. Phaser × Colyseus IntegrationsGuide**

```md
# Phaser × Colyseus IntegrationsGuide

## Architektur
Phaser Scene → colyseusClient.ts → Colyseus Server → Rooms → Schema

## StartIslandScene
```ts
async create() {
  await colyseusClient.connectChatRoom("Frederik");

  colyseusClient.onNewMessage = (msg) => {
    this.chatOverlay.appendMessage(msg);
  };

  this.chatOverlay = new ChatOverlay(this);
}
this.chatOverlay.onSend = (text) => {
  colyseusClient.sendChatMessage(text);
};
...
Ergebnis
Multiplayer‑Startinsel + Echtzeit‑Chat + saubere Architektur.


---

# # **FERTIG. Alles in EINEM Markdown. Alles Copy‑Paste‑ready.**
