...
exports.terminateTokenSwap = void 0;
/*
...
async function terminateTokenSwap(ctx, swapRequestId) {
    const transactionTime = ctx.txUnixTime;
    const chainValidationErrors = [];
    // This will throw an error if it can't be found
    const tokenSwap = await (0, utils_1.getObjectByKey)(ctx, api_1.TokenSwapRequest, swapRequestId);
    if (tokenSwap.offeredBy !== ctx.callingUser) {
        chainValidationErrors.push(`${ctx.callingUser} does not have permission to terminate token swap ${swapRequestId}`);
    }
    ...
    for (const instanceQuantity of tokenSwap.offered) {
        ...
        await (0, locks_1.unlockToken)(ctx, {
            ...
            chainValidationErrors.push(`UnlockToken() failed for token ${instanceQuantity.tokenInstance.toStringKey()}, ` +
    ...
    if (chainValidationErrors.length === 0) {
        ...
        await (0, utils_1.putChainObject)(ctx, tokenSwap);
        ...
        throw new SwapError_1.SwapDtoValidationError("terminateTokenSwap", chainValidationErrors);
    }
}
exports.terminateTokenSwap = terminateTokenSwap;
//# sourceMappingURL=terminateTokenSwap.js.map