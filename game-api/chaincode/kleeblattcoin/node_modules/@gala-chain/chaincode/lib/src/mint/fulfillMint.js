...
async function mintRequestsByTimeRange(ctx, tokenClass, startTimestamp, endTimestamp) {
    ...
    const mostRecentTime = endTimestamp !== null && endTimestamp !== void 0 ? endTimestamp : ctx.txUnixTime - utils_1.blockTimeout;
    const oldestTime = startTimestamp !== null && startTimestamp !== void 0 ? startTimestamp : 0;
    const recentTimeKey = (0, utils_1.generateInverseTimeKey)(mostRecentTime);
    const oldestTimeKey = (0, utils_1.generateInverseTimeKey)(oldestTime);
...
async function mintRequestsByTimeKeys(ctx, tokenClass, recentTimeKey, oldestTimeKey) {
    ...
    const startKey = [
        ...
        recentTimeKey,
        "".padStart(utils_1.inverseKeyLength, "0")
    ...
    const endKey = [
        ...
        tokenClass.additionalKey,
        oldestTimeKey,
        "".padStart(utils_1.inverseKeyLength, "z")
    ].join(api_1.ChainObject.MIN_UNICODE_RUNE_VALUE);
    ...
    try {
        ...
        throw new api_1.RuntimeError(`Error encountered while processing ctx.stub.getStateByRange Async Iterator for ` +
...
function validateMintRequestDtoMatchesOriginal(mintRequestDto, originalRequest) {
    var _a, _b;
    const fieldsToVerify = [
        ...
        [
            ...
            (_a = mintRequestDto.totalKnownMintsCount) === null || _a === void 0 ? void 0 : _a.toFixed(),
            (_b = originalRequest.totalKnownMintsCount) === null || _b === void 0 ? void 0 : _b.toFixed()
        ]
    ];
...
async function fulfillMintRequest(ctx, { requests, callingUser }) {
    var _a;
    ...
    for (const [, values] of Object.entries(reqIdx)) {
        ...
        const tokenKey = api_1.ChainObject.getCompositeKeyFromParts(api_1.TokenClass.INDEX_KEY, [
            collection,
            category,
            type,
            additionalKey
        ...
        let mostRecentTimeInversion = new bignumber_js_1.default(utils_1.inversionHeight), oldestTimeInversion = new bignumber_js_1.default("0");
        ...
        const recentTimeKey = mostRecentTimeInversion.toString().padStart(utils_1.inverseKeyLength, "0");
        const oldestTimeKey = oldestTimeInversion.toString().padStart(utils_1.inverseKeyLength, "0");
        ...
        if (requestEntries.length < 1) {
            throw api_1.GalaChainResponse.Error(new Error(`FulfillMint failure: No TokenMintRequest(s) found on chain. ` +
        ...
        for (let i = 0; i < requestEntries.length; i++) {
            ...
            if (!mintableQty) {
                ...
                const mintReqParams = {
                    tokenClass: (0, class_transformer_1.plainToInstance)(api_1.TokenClassKey, { collection, category, type, additionalKey }),
                    owner: req.owner,
                ...
                const applicableAllowances = await (0, validateMintRequest_1.validateMintRequest)(ctx, mintReqParams, tokenClass, callingUser);
                const actionDescription = `${api_1.AllowanceType.Mint} ${req.quantity.toString()} ` +
                    `token ${dtoInstanceKey.toStringKey()} to ${req.owner}`;
                const allowancesUsed = await (0, allowances_1.useAllowances)(ctx, new bignumber_js_1.default(req.quantity), applicableAllowances, api_1.AllowanceType.Mint);
                if (!allowancesUsed) {
                    failures.push({
                        req: req,
                ...
                try {
                    const [instances, balance] = await (0, constructVerifiedMints_1.constructVerifiedMints)(ctx, mintFulfillmentEntry, tokenClass, instanceCounter, callingUser);
                    const returnKeys = [];
...
//# sourceMappingURL=fulfillMint.js.map