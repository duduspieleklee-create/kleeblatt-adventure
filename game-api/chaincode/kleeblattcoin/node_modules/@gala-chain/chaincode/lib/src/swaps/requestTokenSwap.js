...
exports.requestTokenSwap = void 0;
/*
...
function validateTokenSwapQuantity(quantity, tokenClass) {
...
async function requestTokenSwap(ctx, { offeredBy, offeredTo, offered, wanted, uses, expires }) {
    const newSwap = new api_1.TokenSwapRequest();
    newSwap.offeredBy = (0, api_1.asValidUserAlias)(offeredBy);
    newSwap.offeredTo = offeredTo ? (0, api_1.asValidUserAlias)(offeredTo) : undefined;
    newSwap.offered = offered.map((t) => {
        const tokenInstanceQty = new api_1.TokenInstanceQuantity();
        const key = new api_1.TokenInstanceKey();
    ...
    newSwap.txid = ctx.stub.getTxID();
    newSwap.uses = uses;
    newSwap.usesSpent = new bignumber_js_1.BigNumber(0);
    ...
    const chainValidationErrors = [];
    let offeredOnBehalfOfOwner = false;
    let verifyAuthorizationForLock = async () => undefined;
    ...
    if (newSwap.expires && newSwap.expires <= ctx.txUnixTime) {
        chainValidationErrors.push(`If expiry is provided it must be a valid epoch time in the future: ${newSwap.expires}`);
    }
    // Check that token IDs exist
    const offeredInstanceKeys = newSwap.offered.map((t) => t.tokenInstance);
    const offeredInstances = await (0, token_1.fetchTokenInstances)(ctx, offeredInstanceKeys).catch((e) => {
    ...
    for (let index = 0; index < newSwap.offered.length; index += 1) {
        ...
        const tokenClassKeyList = api_1.TokenClass.buildClassKeyList(tokenInstanceKey);
        const compositeKey = api_1.TokenClass.getCompositeKeyFromParts(api_1.TokenClass.INDEX_KEY, tokenClassKeyList);
        ...
        if (offeredOnBehalfOfOwner) {
            ...
            const instanceCompositeKey = api_1.TokenInstance.getCompositeKeyFromParts(api_1.TokenInstance.INDEX_KEY, tokenInstanceKeyList);
            const tokenInstance = await (0, utils_1.getObjectByKey)(ctx, api_1.TokenInstance, instanceCompositeKey);
            await (0, allowances_1.verifyAndUseAllowances)(ctx, newSwap.offeredBy, tokenInstanceKey, quantity, tokenInstance, ctx.callingUser, api_1.AllowanceType.Swap);
        }
    ...
    for (let index = 0; index < newSwap.wanted.length; index++) {
        ...
        const tokenInstanceKeyList = api_1.TokenClass.buildClassKeyList(tokenInstance);
        const compositeKey = api_1.TokenClass.getCompositeKeyFromParts(api_1.TokenClass.INDEX_KEY, tokenInstanceKeyList);
        const tokenClass = await (0, utils_1.getObjectByKey)(ctx, api_1.TokenClass, compositeKey);
        if (tokenClass.isNonFungible === false && fungibleTokenKeys.includes(compositeKey)) {
            chainValidationErrors.push(`Error validating tokens to be swapped: Can not swap two of the same fungible tokens.`);
        }
    ...
    if (chainValidationErrors.length === 0) {
        ...
        for (let index = 0; index < newSwap.offered.length; index += 1) {
            ...
            const balance = await (0, balances_1.fetchOrCreateBalance)(ctx, newSwap.offeredBy, fromToken);
            ...
            if (!isSpendable) {
                ...
                chainValidationErrors.push(`${fromPersonKey} has insufficient balance to spend ${offeredQuantity} of token ${fromToken.toStringKey()}`);
            }
        }
    }
    // Persist the swap request
    await (0, utils_1.putChainObject)(ctx, newSwap);
    const swapRequestId = newSwap.getCompositeKey();
    if (chainValidationErrors.length !== 0) {
        throw new SwapError_1.SwapDtoValidationError("requestTokenSwap", chainValidationErrors);
    }
    ...
    for (let index = 0; index < newSwap.offered.length; index += 1) {
        ...
        await (0, locks_1.lockToken)(ctx, {
            ...
            throw new locks_1.LockTokenFailedError(chainError.message, chainError.payload);
        });
    }
    ...
    for (const offered of newSwap.offered) {
        const newSwapInstance = (0, class_transformer_1.plainToInstance)(api_1.TokenSwapRequestInstanceOffered, {
            ...offered.tokenInstance,
            swapRequestId: swapRequestId
        });
        await newSwapInstance.validateOrReject();
        await (0, utils_1.putChainObject)(ctx, newSwapInstance);
    }
    for (const wanted of newSwap.wanted) {
        const newSwapInstance = (0, class_transformer_1.plainToInstance)(api_1.TokenSwapRequestInstanceWanted, {
    ...
    const newSwapOfferedBy = (0, class_transformer_1.plainToInstance)(api_1.TokenSwapRequestOfferedBy, {
        offeredBy: newSwap.offeredBy,
        swapRequestId: swapRequestId
    });
    await newSwapOfferedBy.validateOrReject();
    await (0, utils_1.putChainObject)(ctx, newSwapOfferedBy);
    // offering to a specfic user is not required
    if (newSwap.offeredTo) {
        const newSwapOfferedTo = (0, class_transformer_1.plainToInstance)(api_1.TokenSwapRequestOfferedTo, {
            offeredTo: newSwap.offeredTo,
            swapRequestId: swapRequestId
        });
        await newSwapOfferedTo.validateOrReject();
        await (0, utils_1.putChainObject)(ctx, newSwapOfferedTo);
    }
    return newSwap;
}
exports.requestTokenSwap = requestTokenSwap;
//# sourceMappingURL=requestTokenSwap.js.map