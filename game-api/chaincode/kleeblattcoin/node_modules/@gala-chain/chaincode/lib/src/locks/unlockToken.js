...
async function unlockToken(ctx, { tokenInstanceKey, name, quantity, owner }) {
    ...
    if (tokenInstance.owner === undefined) {
        throw new api_1.RuntimeError(`Token instance ${tokenInstanceKey.toStringKey()} has no owner`);
    }
    const balance = await (0, balances_1.fetchOrCreateBalance)(ctx, tokenInstance.owner, tokenInstanceKey.getTokenClassKey());
    ...
    if (!authorized) {
        const tokenClass = await (0, token_1.fetchTokenClass)(ctx, tokenInstanceKey);
...
async function unlockFungibleToken(ctx, { tokenInstanceKey, name, quantity, owner }) {
    owner = owner !== null && owner !== void 0 ? owner : ctx.callingUser;
