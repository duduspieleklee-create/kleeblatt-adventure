...
exports.getApiMethod = exports.getApiMethods = exports.updateApi = void 0;
/*
...
// eslint-disable-next-line @typescript-eslint/ban-types
target, methodName, additionalFilter = () => true) {
    const apiMethods = getApiMethods(target).filter(additionalFilter);
    const method = apiMethods.find((m) => (m.apiMethodName && m.apiMethodName === methodName) || m.methodName === methodName);
    if (method) {
        return method;
    }
    // check if method exists
    const availableMethods = apiMethods.map((m) => { var _a; return (_a = m.apiMethodName) !== null && _a !== void 0 ? _a : m.methodName; }).sort();
    throw new api_1.NotFoundError(`Method ${methodName} is not available. Available methods: ${availableMethods.join(", ")}`);
}
exports.getApiMethod = getApiMethod;
//# sourceMappingURL=GalaContractApi.js.map