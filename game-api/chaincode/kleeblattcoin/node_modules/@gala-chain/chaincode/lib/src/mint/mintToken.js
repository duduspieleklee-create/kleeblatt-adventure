...
exports.updateTotalSupply = exports.mintToken = exports.parseMintTokenParams = void 0;
/*
...
function parseMintTokenParams(plain) {
    const tokenClassKey = api_2.ChainCallDTO.deserialize(api_1.TokenClassKey, plain.tokenClassKey);
    const quantity = plain.quantity instanceof bignumber_js_1.BigNumber ? plain.quantity : new bignumber_js_1.BigNumber(plain.quantity);
    const knownTotalSupply = plain.knownTotalSupply === undefined
        ? undefined
        : plain.knownTotalSupply instanceof bignumber_js_1.BigNumber
            ? plain.knownTotalSupply
            : new bignumber_js_1.BigNumber(plain.knownTotalSupply);
    return {
        tokenClassKey,
        owner: plain.owner,
        quantity,
        authorizedOnBehalf: plain.authorizedOnBehalf,
        knownTotalSupply
    };
}
exports.parseMintTokenParams = parseMintTokenParams;
async function mintToken(ctx, { tokenClassKey, owner, quantity, authorizedOnBehalf, knownTotalSupply }) {
    var _a, _b;
    ...
    const fetchAllowancesData = {
        ...
        instance: api_1.TokenInstance.FUNGIBLE_TOKEN_INSTANCE.toString(),
        allowanceType: api_1.AllowanceType.Mint
    };
    let applicableAllowanceResponse = await (0, allowances_1.fetchAllowances)(ctx, fetchAllowancesData);
    // Filter allowances to only include those granted by current authorities
    applicableAllowanceResponse = applicableAllowanceResponse.filter((allowance) => tokenClass.authorities.includes(allowance.grantedBy));
    ...
    const allowancesUsed = await (0, allowances_1.useAllowances)(ctx, new bignumber_js_1.BigNumber(quantity), applicableAllowanceResponse, api_1.AllowanceType.Mint);
    if (!allowancesUsed) {
...
async function updateTotalSupply(ctx, { tokenClassKey, callingUser, owner, quantity, knownTotalSupply }) {
